...
def deepmoji_architecture(
    ...
    lstm_1_output = Bidirectional(LSTM(512, return_sequences=True), name="bi_lstm_1")(
        lstm_0_output
    )
...
def load_specific_weights(
    ...
    for i, w in enumerate(layer_weights):
        ...
        try:
            ...
            raise ValueError(
                "Weights had layer {}, but could not find this layer in model.".format(
                    l_name
                )
            )
        ...
        if type(model_l) is Embedding and extend_embedding > 0:
            comb_weights = append_to_embedding(weight_values, model_l.get_weights())
            model_l.set_weights(comb_weights)
            if verbose:
                print(
                    "Extended vocabulary for embedding layer from {} to {} tokens.".format(
                        NB_TOKENS, NB_TOKENS + extend_embedding
                    )
                )
        else:
