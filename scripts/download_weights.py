...
WEIGHTS_DOWNLOAD_LINK = (
    "https://dl.dropboxusercontent.com/s/xqarafsl6a8f9ny/deepmoji_weights.hdf5"
)
WEIGHTS_FILE_SHA_256 = (
    "ca663315cf4a22ced569cf928f9277ebd013c55d93a19e1d0dde001f59a72476"
)
...
if os.path.exists(WEIGHTS_PATH):
    print(
        "Weight file already exists at {}. Would you like to redownload it anyway? [y/n]".format(
            WEIGHTS_PATH
        )
    )
    download = prompt()
...
if download:
    print(
        "About to download the pretrained weights file from: {}".format(
            WEIGHTS_DOWNLOAD_LINK
        )
    )
    ...
    if already_exists or prompt():
        ...
        if m.hexdigest() != WEIGHTS_FILE_SHA_256:
            raise ValueError(
                "Downloaded weights sha256sum: {} is not the expected: {}".format(
                    m.hexdigest(), WEIGHTS_FILE_SHA_256
                )
            )
